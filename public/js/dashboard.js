...
function setupMicLongPress(micButton) {
    if (!micButton) {
        micButton = document.getElementById('voiceButtonMobile');
        if (!micButton) return;
    }
...
function handleMicPress() {
    ...
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    const longPressDelay = isMobile ? 300 : 500; // Faster response on mobile
    
    // Start long press timer
    micLongPressTimer = setTimeout(() => {
        isLongPressActive = true;
        console.log('🎤 Long press activated - continuous recording mode');
        startContinuousRecording();
    }, longPressDelay);
    ...
    if (micButton) {
        ...
        micButton.style.transition = 'transform 0.1s ease';
    }
}
...
function handleMicRelease() {
    ...
    if (isLongPressActive) {
        // Long press mode - stop recording and send
        console.log('🎤 Long press ended - sending final transcript');
        stopContinuousRecording();
        sendFinalTranscript();
    } else {
        // Short press mode - start normal recording
        console.log('🎤 Short press - starting normal recording');
        startNormalRecording();
    }
}
...
function initEnhancedSpeechRecognition() {
    ...
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    if (isMobile) {
        voiceRecognition.continuous = false; // Better for mobile
        voiceRecognition.interimResults = true; // Enable for better responsiveness
        voiceRecognition.maxAlternatives = 2; // Reduced for mobile efficiency
    }
    ...
    voiceRecognition.onresult = (event) => {
        ...
        displayRealTimeTranscript(currentTranscript);
        
        // For short press mode, send when there's a pause
        if (!isLongPressActive && finalTranscript.trim()) {
            setTimeout(() => {
                if (!isRecording) {
    ...
    voiceRecognition.onerror = (event) => {
        ...
        if (event.error === 'no-speech') {
            ...
            if (isMobile) {
                showError('Microphone access denied. Please go to Settings > Privacy > Microphone and enable access for this app.');
            } else {
            ...
            if (isMobile) {
                showError('Voice recognition error. Please try again or restart the app.');
            } else {
                showError('Voice recognition error. Please try again.');
            }
        }
...
function displayRealTimeTranscript(transcript) {
    // Display transcript in a temporary area for user to see
    const chatInput = document.getElementById('chatInput');
    const chatInputMobile = document.getElementById('chatInputMobile');
    ...
    // Show visual indicator that we're listening
    showListeningIndicator();
}

function showListeningIndicator() {
    // Add a small indicator that we're actively listening
    const indicator = document.createElement('div');
    indicator.id = 'listeningIndicator';
    indicator.className = 'fixed top-4 right-4 bg-red-500 text-white px-3 py-1 rounded-full text-xs animate-pulse';
    indicator.textContent = '🎤 Listening...';
    document.body.appendChild(indicator);
    
    // Remove indicator after 3 seconds
    setTimeout(() => {
        const existingIndicator = document.getElementById('listeningIndicator');
...
function sendFinalTranscript() {
    if (currentTranscript.trim()) {
        console.log('🎤 Sending final transcript:', currentTranscript);
    ...
    if (chatInputMobile) {
        ...
        chatInputMobile.placeholder = 'Ask me anything about your studies...';
    }
}

function sendTranscriptToAI(transcript) {
