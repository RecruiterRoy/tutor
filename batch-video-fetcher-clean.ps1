...
function Start-BatchProcessing {
    ...
    try {
        ...
        if ($response.success) {
            ...
            Write-ColorOutput "⏳ Processing in background... Check progress with: .\batch-video-fetcher-clean.ps1 -Action get_progress" "Yellow"
        } else {
...
switch ($Action.ToLower()) {
    ...
    "process_specific" {
        if (-not $Subject -or -not $ClassLevel) {
            ...
            Write-ColorOutput "Usage: .\batch-video-fetcher-clean.ps1 -Action process_specific -Subject english -ClassLevel 1-3" "Yellow"
            exit 1
        }
    ...
    default {
        ...
        Write-ColorOutput "   .\batch-video-fetcher-clean.ps1 -Action start_all" "White"
        Write-ColorOutput "   .\batch-video-fetcher-clean.ps1 -Action get_progress" "White"
        Write-ColorOutput "   .\batch-video-fetcher-clean.ps1 -Action process_specific -Subject english -ClassLevel 1-3" "White"
    }
}
